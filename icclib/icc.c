 ...
 *      weren't being handled properly throught the library.
 *
 * 2.02
...
...
	...
	/* Should use :
	   ...
	   but it doesn't seem widely enough supported. */

	/* This could overwrite the buffer !!! */
	rv = vsprintf(p->cur, format, args);
...
/* Public: */
int read_Primitive(icmPrimType ptype, void *prim, char *p) {
	...
	switch(ptype) {
		...
		case icmS15Fixed16Number:
			*((double *)prim) = read_S15Fixed16Number(p);
			return 0;
		case icmPCSXYZ16Number:
			*((double *)prim) = read_PCSXYZ16Number(p);
			return 0;
		case icmPCSL8Number:
			*((double *)prim) = read_PCSL8Number(p);
			return 0;
		case icmPCSab8Number:
			*((double *)prim) = read_PCSab8Number(p);
			return 0;
		case icmPCSL16Number:
			*((double *)prim) = read_PCSL16Number(p);
			return 0;
		case icmPCSab16Number:
			*((double *)prim) = read_PCSab16Number(p);
			return 0;
		case icmDCS8Number:
...
/* Public: */
int write_Primitive(icmPrimType ptype, char *p, void *prim) {
	...
	switch(ptype) {
		...
		case icmPCSXYZ16Number:
			return write_PCSXYZ16Number(*((double *)prim), p);
		case icmPCSL8Number:
			return write_PCSL8Number(*((double *)prim), p);
		case icmPCSab8Number:
			return write_PCSab8Number(*((double *)prim), p);
		case icmPCSL16Number:
			return write_PCSL16Number(*((double *)prim), p);
		case icmPCSab16Number:
			return write_PCSab16Number(*((double *)prim), p);
		case icmDCS8Number:
...
static struct {
	...
	{icSigMch5Data,    Lut_5,        Lut_5,         Lut_5,        Lut_5 },
	{icSigMch6Data,    Lut_6,        Lut_6,         Lut_6,        Lut_6 },
	{icSigMch7Data,    Lut_7,        Lut_7,         Lut_7,        Lut_7 },
	{icSigMch8Data,    Lut_8,        Lut_8,         Lut_8,        Lut_8 },
